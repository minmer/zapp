...
export default function ConfirmationChatAdminSubpage({ getParams }: { getParams: ({ func, type, show }: { func: (p: string| User) => Promise<unknown>, type: string, show: boolean }) => Promise<unknown> }) {
    ...
    return (
        <div className="confirmation-group-chat">
            ...
            <ChatElement getParams={getParams} name={'confirmation_' + role?.roleID} viewer={role?.roleID + 'common'} writer={adminRole ? (adminRole.roleID) : (role?.roleID)} alias={alias} />
        </div>
    );
}